(function () {
  ...
  const state = { week: params.get('week') || '', data: null, printing: params.get('print') === '1' };
  ...
  async function load(week = '') {
    ...
    try {
      ...
      const response = await fetch(`/api/lesson-plans/public/${encodeURIComponent(token)}${query}`, { headers: { Accept: 'application/json' } });
