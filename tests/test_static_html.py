...
class StaticHtmlTestCase(unittest.TestCase):
    ...
    def test_ide_search_does_not_force_wiki_mode_and_hides_contents_toggle(self):
        ...
        self.assertIn("scrollToArticleTarget", reader)
        self.assertIn("root.scrollTop + targetRect.top - rootRect.top", reader)
        self.assertIn("requestIdleCallback", reader)
        ...
        self.assertIn(".wiki-embedded-media video", wiki_css)
        self.assertIn("body.wiki-mode #wikiNavBtn", wiki_css)
        feature_loader = (BASE_DIR / "static" / "js" / "feature-loader.js").read_text(encoding="utf-8")
        tools_js = (BASE_DIR / "static" / "js" / "wiki-tools.js").read_text(encoding="utf-8")
        self.assertIn("wikiTools", feature_loader)
        self.assertIn("/static/css/features/wiki-tools.css", feature_loader)
        self.assertNotIn("features/wiki-tools.css", (BASE_DIR / "static" / "css" / "main.css").read_text(encoding="utf-8"))
        self.assertIn("disposeWikiContent", reader)
        self.assertIn("media.pause()", reader)
        self.assertIn("media.removeAttribute('src')", reader)
        self.assertIn("window.EagleFeatures?.load?.('wikiTools')", reader)
        self.assertIn("calculateSubnet", tools_js)
        self.assertIn('id="wikiAdminNewToolBtn"', self.raw)
        self.assertIn('id="wikiToolPickerModal"', self.raw)
        app_core = (BASE_DIR / "static" / "js" / "app-core.js").read_text(encoding="utf-8")
