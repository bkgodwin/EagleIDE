...
There is no frontend build system, package manager, database, or separate API service. The browser UI uses plain HTML, CSS, and global JavaScript. Application data is stored in JSON, CSV, and per-user directories beside the source tree.
...
3. Classroom, notebook, and student dashboard feature scripts, which consume state exposed through `window.EagleIDE`.
